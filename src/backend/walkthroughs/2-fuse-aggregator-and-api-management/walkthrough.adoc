...
:fuse-flights-aggregator-app-name: flights-{user-sanitized-username}
...
.. Select the *(1) Application* item from the breadcrumb to view Applications.
...
. Select the *Analytics > Traffic* from the side menu
. Make sure the *Hits (hits)* metric is selected in the dropdown
